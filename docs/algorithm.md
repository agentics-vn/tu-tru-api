...
Công thức: `ba ngày ≈ một năm khởi vận` → `tuổi_khởi_vận = max(1, ceil(số_ngày / 3))` (làm tròn **lên** — khớp với quy ước lịch vạn niên truyền thống Việt Nam; `round()` sẽ cho kết quả lệch 1 tuổi khi số ngày không chia hết cho 3).
...
So tuổi đã tròn (năm − năm sinh, trừ 1 nếu chưa tới sinh nhật trong năm dương) với khoảng `[start_age, end_age]` của từng đại vận (`start_age` của vận 1 = tuổi khởi vận; mỗi vận 10 năm, `end_age = start_age + 9`). Hiển thị theo quy ước truyền thống: `age_range = "{start_age}-{start_age+10}"` (ví dụ "36-46" = từ tuổi 36 đến hết tuổi 45).
...
- **Health:** `GET /health` trả `version` (FastAPI app) + `engine_version` (từ `pyproject.toml` hoặc `0.1.3` trong Docker).
