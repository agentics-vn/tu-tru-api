...
| 422 | NO_DATES_FOUND | ~~0 dates passed all layers~~ **Deprecated** — empty scan returns **200** + `empty_reason_vi` (Direction C) |
...
## Direction C contract (NLTT consumer)

### Breaking change — `POST /v1/chon-ngay` empty

| Before | After |
|---|---|
| HTTP **422** `NO_DATES_FOUND` | HTTP **200**, `ranked_days: []`, `empty_reason_vi` (Vietnamese) |

Top-level fields: `intent`, `intent_label_vi`, `range_start`, `range_end` (ISO), `ranked_days[]`, `score_methodology`, `empty_reason_vi`.

Row `reason_vi` = `summary_vi` only (never joined `reasons_vi[]`).

### `GET /v1/day-detail`

| Param | Notes |
|---|---|
| `mode` | `generic` (no birth) \| `personalized` (default) |
| `intent` | Default `MAC_DINH`; used for scoring in personalized mode |
| `date` | `YYYY-MM-DD` |
| `birth_date` | `dd/mm/yyyy` when personalized |

Response: `breakdown[]` (4 items), `sources[]`, `score_methodology`, `sum(breakdown[].points) == score`.

### `GET /v1/day-detail/luan-context`

LLM facts — `breakdown_summary[]` mirrors `day-detail.breakdown` for same inputs.

`gio_tot` / `gio_xau` use canonical slot objects (P3-02). For clients that previously consumed `string[]`, use **`gio_tot_labels`** / **`gio_xau_labels`** (`label_vi` per slot).

### `GET /v1/day-compare`

`delta_score = score_b - score_a` (signed).

### Timezone / dates

- Default timezone: `Asia/Ho_Chi_Minh` via `tz` query param.
- Target dates: `YYYY-MM-DD`; birth dates: `dd/mm/yyyy`.

### Fixtures

Golden JSON: `docs/fixtures/direction-c/` — regenerate via `python scripts/generate_direction_c_fixtures.py`.

### Staging

Set NLTT `BAT_TU_API_URL` to Fly staging/preview URL before QA (see README).

### P3 enhancements

| ID | Contract |
|----|----------|
| P3-01 | `GET /health` → `{ status, version, engine_version }` |
| P3-02 | `gio_tot` / `gio_xau` slots: `{ chi, chi_name, start_hour, end_hour, label_vi, range }` on day-detail, ngay-hom-nay, chon-ngay, lich-thang |
| P3-03 | `purpose_rows[]` on personalized `day-detail` (per-intent `verdict` + `reason_vi`) |
| P3-04 | Response headers `X-RateLimit-Limit`, `X-RateLimit-Remaining` (env `RATE_LIMIT_PER_MINUTE`, default 300) |
