...
def solar_apparent_longitude_deg(dd: int, mm: int, yy: int, time_zone: float = 7.0) -> float:
    """
    Sun's apparent ecliptic longitude in degrees [0, 360), for local calendar date.
    Uses the same model as getSunLongitude (Meeus), without the 12-segment quantization.
    """
    jdn = jdFromDate(dd, mm, yy)
